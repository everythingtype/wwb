...
function send_request_via_curl($host,$path,$content)
{
	...
	do_action('curl_response', $posturl, $content, $response);
	return $response;
}
...
{
	$errored = substring_between($content,'<errorCode>','</errorCode>');
	if (!$errored) $resultCode = substring_between($content,'<resultCode>','</resultCode>');		
	else $resultCode = 'Error';
	...
	$subtext = substring_between($content,'<errorText>','</errorText>');
	if ( $subtext ) $text = $text . ' ('.$subtext.')';
	$subscriptionId = substring_between($content,'<subscriptionId>','</subscriptionId>');
	if (!$subscriptionId) $subscriptionId = substring_between($content,'<transId>','</transId>');
